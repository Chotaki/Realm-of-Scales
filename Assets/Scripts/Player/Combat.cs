...
public class Combat : MonoBehaviour
{
    ...
    [SerializeField] private AudioClip playerCast;
    ...
    void Update()
    {
        if(Time.time >= _nextAttackTime)
        {
            ...
            {
                SFXManager.instance.PlaySFXClip(playerSwing, transform, 1f);
            ...
            {
                throwFireBall();
                SFXManager.instance.PlaySFXClip(playerCast, transform, 1f);
                _nextAttackTime = Time.time + 1f / _attackRate;
            }
        }
